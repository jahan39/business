/**
 * ============================================================
 * PRODUCT MANAGER — localStorage-based CRUD for Sugar Atelier
 * ============================================================
 * Handles: Add, Edit, Delete, List products
 * Products stored in localStorage key: "sugarAtelierProducts"
 * ============================================================
 */
...
// ================================================================
// ADMIN PRODUCTS PAGE — render product table from localStorage
// ================================================================
  ...
  function renderAdminTable() {
    ...
    tbody.querySelectorAll(".delete-product").forEach(btn => {
      btn.onclick = () => {
        ...
        const saved = getSavedProducts().filter(p => p.id !== btn.dataset.id);
...
function initAdminProductForm() {
  ...
  fileInput?.addEventListener("change", () => {
    ...
    if (!file) { if (previewEl) previewEl.style.display = "none"; return; }
  ...
  form.addEventListener("submit", e => {
    ...
    // Helper to build & save product with a resolved image string
    function saveProduct(imageStr) {
      const productData = {
        id:          editProduct ? editProduct.id : genId(),
        name,
        sku:         fd.get("sku")?.trim() || ("SC-" + Date.now()),
        category,
        price,
    ...
    if (file) {
      // User selected a file → convert to base64 and save
      const reader = new FileReader();
      reader.onload = ev => saveProduct(ev.target.result);
      reader.readAsDataURL(file);
    } else {
      // No file → use URL field or keep existing image or use default
...
// ================================================================
// SHOP PAGE — inject localStorage products into the product grid
// ================================================================
function injectShopProducts() {
  ...
  saved.forEach(p => {
    // Skip if already rendered (by name match)
    const safeName = p.name.replace(/"/g, '\\"');
    if (grid.querySelector(`[data-name="${safeName}"]`)) return;

    const article = document.createElement("article");
    article.className = "product-card searchable";
  ...
  // Also add any new categories from saved products to the pill filters
  ...
  if (pillsContainer) {
    ...
    saved.forEach(p => {
      ...
      if (cat && !existingCats.has(cat)) {
        ...
        // Re-attach event (the shop filter script handles delegation via re-init)
      }
    });
  }
}
...
function injectHomepageProducts() {
  ...
  // Combine builtin + saved, show up to 3 newest saved (or fallback to builtins)
  const saved = getSavedProducts().filter(p => p.status !== "Draft");
  const toShow = saved.length ? saved.slice(-3).reverse() : [];
  if (!toShow.length) return;

  // Remove existing hardcoded cards and replace with dynamic
  toShow.forEach(p => {
    if (grid.querySelector(`[data-name="${p.name.replace(/"/g, '\\"')}"]`)) return;
    const hasBadge = p.discount && p.discount > p.price;
    const discPct  = hasBadge ? Math.round((1 - p.price / p.discount) * 100) : 0;
    const article  = document.createElement("article");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Re-run shop filter count AFTER products are injected
  // (app.js initShop runs first on DOMContentLoaded, so count may be stale)
  const grid = document.getElementById("shopGrid");
  if (grid) {
